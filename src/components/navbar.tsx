      ...
      <nav className="bg-dm-black p-8">
        <div className="max-w-7xl mx-auto flex items-center justify-center gap-4">
          <Link href="/" className="flex flex-1 items-center font-headings text-xl">
            <div className="flex items-center gap-4 bg-artic-blue rounded-full p-1 pr-3 font-semibold cursor-pointer">
          ...
          <div className="flex flex-1 items-center justify-end gap-4 font-medium font-main text-lg">
            <button
              type="button"
              className="bg-artic-blue rounded-full p-1 px-3 cursor-pointer hover:bg-blue-600 hover:text-white"
          ...
          <nav className="flex gap-8 leading-6 font-headings text-xl" aria-label="Primary">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="flex items-center gap-4 bg-dm-black text-white rounded-full py-1 px-6 cursor-pointer border-4 border-dm-black hover:bg-blue-600"
              >
                {link.label}
              </Link>
            ))}
